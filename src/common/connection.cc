...
using zmq::socket_t;
void* MyEntry(void* ptr){
    printf("new thread is created.\n");
    ConnectionMultiplexer* caller = (ConnectionMultiplexer*)ptr;
    caller->Run();
    return NULL;
}
...
pthread_attr_setaffinity_np(&attr, sizeof(cpu_set_t), &cpuset);
  ...
  //int ret=pthread_create(&thread_, &attr, RunMultiplexer, reinterpret_cast<void*>(this));
  int ret=pthread_create(&thread_,NULL,&RunMultiplexer,(void*)this); 
  if (ret){
    errno=ret;
